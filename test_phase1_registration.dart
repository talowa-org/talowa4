...
Future<void> validateRegistrationFlow() async {
  ...
  print(
    referralResult
        ? '✅ PASS - Referral activation works'
        : '❌ FAIL - Referral activation failed',
  );
  ...
  print(
    statsResult
        ? '✅ PASS - Referral statistics work'
        : '❌ FAIL - Referral statistics failed',
  );
...
Future<bool> testFormSubmission() async {
  try {
    ...
    // Generate test data
    final testPhone =
        '+919876543${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    final testData = {
