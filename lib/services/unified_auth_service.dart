...
class UnifiedAuthService {
  ...
  static Future<AuthResult> registerUser({
    ...
    try {
      ...
      try {
        generatedReferralCode = await ReferralCodeGenerator.generateUniqueCode();
        debugPrint('Generated referral code: $generatedReferralCode');
      } catch (e) {
        debugPrint('Failed to generate referral code: $e');
        // Use fallback code if generation fails
        generatedReferralCode = 'TAL${DateTime.now().millisecondsSinceEpoch.toString().substring(0, 6)}';
      }
